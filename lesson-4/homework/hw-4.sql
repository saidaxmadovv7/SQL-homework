...
----Write a query to select the top 5 employees from the Employees table.--savoli
select Top 5 Firstname from Employees
order by Salary desc


---Write a query that uses the WHERE clause to filter for employees with Salary >= 60000 and Department = 'HR'--savoli.
 select *from Employees
 where Salary>=6000 and DepartmentName='HR'

 ---Use ISNULL to replace NULL values in the Email column with the text "noemail@example.com".From Employees table--savoli.

 select isnull(Email,'noemail@example.com') as Email from Employees

 ---Use COALESCE to return the first non-NULL value from FirstName or LastName in the Employees table.---savoli.

 select coalesce (Firstname,LastName) from Employees

 --Write a query that filters the Employees table to show employees whose Age is either between 30 and 40 or Department = 'Marketing'.--savoli.
 ...
 ---Use OFFSET-FETCH to select rows 11 to 20 from the Employees table, ordered by Salary DESC.--savoli.
...
---Use IN operator to filter for employees who work in either 'HR', 'IT', or 'Finance'.--savoli.

select *from Employees
where DepartmentName in('HR','IT','Finance')


----Combine FirstName and LastName into one column named FullName in the Employees table. (only in select statement)--savoli

select FirstName+ ' '+LastName as FullName from Employees

----Use WHERE clause to filter for employees whose Age is less than 30 and who work in either the 'HR' or 'IT' department.--savoli.
select *from Employees
where Age<30 and DepartmentName in('HR','IT')



-----Write a query that uses the ALL operator to find employees whose salary is greater than all employees in the 'Sales' department.--savoli.

select *from Employees
where Salary>All(select Salary from  Employees where DepartmentName='Finance')
order by Salary desc
...
----Use SELECT DISTINCT to select unique Category values from the Products table.--savoli
select distinct Category
from Products_Discounted

-----Write a query that filters the Products table to show products with Price > 100.--savoli.
select * from Products_Discounted
where Price > 100


----Write a query to display all products with Price <= 1000 and Stock > 50, sorted by Stock in ascending order.--savoli.

select *from Products_Discounted
where Price<=1000  and StockQuantity>50
order by StockQuantity asc

---Write a query that filters the Products table for ProductName values containing the letter 'e' using LIKE.--savoli.
...
---Write a query that selects the top 5 products with the highest sales, using TOP(5) and ordered by SalesAmount DESC.--savoli.
...
----Order the results of a Products table by Price in ascending order.--savoli.

select *from Products
order by Price asc

---Write a query that shows all products with Price BETWEEN 50 AND 100.---savoli.
...
---Use SELECT DISTINCT on two columns (Category and ProductName) in the Products table.--savoli.
 

 select distinct Category,ProductName
from Products

---After SELECT DISTINCT on two columns (Category and ProductName) Order the results by ProductName in descending order.--savoli.

select *from Products
order by ProductName desc


----Write a query to select the top 10 products from the Products table, ordered by Price DESC.--savoli.

select Top 10 *from Products
order by Price desc


---Write a query that selects distinct Category and Price from the Products table.--savoli.
select *from Products
select distinct Category,Price from Products


----Write a query to select the distinct Category, ProductName, and Price for products that are priced above $50, using DISTINCT on three columns.--savoli.
...
-----(Do some research on how to find average price of all products)---savoli
...
----Write a query to select all Customers whose FirstName start with 'A' using the LIKE operator.--savoli.

select * from Customers
where FirstName like 'A%'


---Use ORDER BY to display a list of customers ordered by City in ascending and PostalCode in descending order.Use customers table--savoli.

select *from Customers
order by City desc, PostalCode asc

---Use LIKE with wildcard to select all customers whose Email contains the domain '@gmail.com'.--savoli.
