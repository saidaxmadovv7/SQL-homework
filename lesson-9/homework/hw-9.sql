...
select EmployeeID,DepartmentName, *from Employees as E
...
select O.OrderID,O.CustomerID,O.OrderDate,P.Amount,P.PaymentDate, *from Orders as O
...
INSERT INTO Productss (ProductID, ProductName, Price, Category, StockQuantity, SupplierID) VALUES
...
*from Productss,Suppliers


select ProductName,SupplierName, *from Productss as P
join Suppliers as S on  P.SupplierID=S.SupplierID

select ProductName,OrderID, *from Productss as p
join Orders as o on p.ProductID=o.ProductID



select ProductName,OrderID,Price, *from Productss as P

join Orders as O on P.ProductID=O.ProductID
where Price>100

select *from Productss as  p
cross join Orders as O on p.StockQuantity


SELECT o.OrderID, o.ProductID, o.Quantity, p.ProductName, p.StockQuantity
FROM Orders o
...
select p.PaymentID, p.OrderID, o.Quantity, pr.Price, p.Amount
from Payments p
join Orders o on p.OrderID = o.OrderID
join Productss pr on o.ProductID = pr.ProductID
...
FROM Productss P  JOIN Categories C ON P.Category=C.CategoryName
...
select Name,CourseName, *from Students,Courses
...
select s.SaleID, c.FirstName, s.ProductID, s.SaleAmount
from Sales s
join Customers c on s.CustomerID = c.CustomerID
where c.Country = 'USA'
