...
select DepartmentName, count(EmployeeID) as numberof_emp from Employees
group by DepartmentName


select DepartmentName, count(*)as  More_5 from Employees
group by DepartmentName
having  count(*)>5


select count(EmployeeID)as HR_emp from Employees
where DepartmentName='HR'


select DepartmentName, max(Salary)as max_price,min(Salary)as min_price from Employees
group by DepartmentName

select DepartmentName ,avg(Salary)as each_dep from Employees
group by DepartmentName
...
select DepartmentName,  sum(Salary)as Sum_saalry ,avg(Salary) as avg_salary from Employees
group by DepartmentName
...
select  min(Price) as Low_Price from  Products
...
select Category,  min(Price) as Low_price,max(Price) as High_Price from Products
group by Category
...
select  count(CustomerID) as Numberof_rows from Customers
...
select  year(OrderDate),month(OrderDate), sum(TotalAmount)as Total_amount ,count(distinct ProductID) as Unique_pro from Orders
group by year(OrderDate),month(OrderDate)
having  count(distinct ProductID)>=2


select year(OrderDate), min(Quantity)as min_quan ,max(Quantity)as max_quan from Orders
...
select ProductID, sum(SaleAmount),avg(SaleAmount)as avg_amount from Sales
group by ProductID

select year( SaleDate), sum(SaleAmount)as total_sales from Sales
