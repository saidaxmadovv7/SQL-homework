...
---Write a query that uses an alias to rename the ProductName column as Name in the Products table.--savoli.

select productName as Name
from Prooducts

---Write a query that uses CASE to create a conditional column that displays 'High' if Price > 1000, and 'Low' if Price <= 1000 from Products table.--savoli.
...
INSERT INTO Cusstomers (CustomerID, FirstName, LastName, Email, Phone, Address, City, State, PostalCode, Country) VALUES
...
---Write a query that uses an alias to rename the Customers table as Client for easier reference.--savoli.
select *from Cusstomers as Clients

----Use UNION to combine results from two queries that select ProductName from Products and ProductName from Products_Discounted.--savoli

select ProductName from Prooducts
union
select ProductName from  Products_Discountedd 


---Write a query to find the intersection of Products and Products_Discounted tables using INTERSECT.--savoli.
...
---Write a query to select distinct customer names and their corresponding Country using SELECT DISTINCT.---savoli.
 
 select distinct FirstName ,Country
 from Cusstomers

 ---Create a conditional column using IIF that shows 'Expensive' if the Price is greater than 1000, and 'Affordable' if less, from Products table.--10savol.

 
select *,IIF(Price>1000, 'Expensive', 'Affordable') from Prooducts


-----------------------------------------------------------------------------------

drop table  Products_Discountedd 
CREATE TABLE Products_Discountedd (
    ProductID INT PRIMARY KEY,
    ProductName VARCHAR(100),
    Price DECIMAL(10, 2),
    Category VARCHAR(50),
...
INSERT INTO Products_Discountedd VALUES
...
select *from Products_Discountedd


----Use IIF to create a column that shows 'Yes' if Stock > 100, and 'No' otherwise (Products_Discounted table, StockQuantity column).--savoli

select *,IIF(StockQuantity>100, 'Yes', 'No') from Products_Discountedd


---8-savol.Use UNION to combine results from two queries that select ProductName from Products and ProductName from Products_Discounted tables.--
...
----Write a query that returns the difference between the Products and Products_Discounted tables using EXCEPT.--9savol.
...
INSERT INTO Saless(SaleID, ProductID, CustomerID, SaleDate, SaleAmount) VALUES
...
--Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' if SaleAmount BETWEEN 200 AND 500, and 'Low Tier' otherwise. (From Sales table)-savoli.
...
----Use EXCEPT to find customers' ID who have placed orders but do not have a corresponding record in the Sales table.--savoli14

select CustomerID from Orderss
except
select CustomerID from  Saless

--Write a query that uses a CASE statement to determine the discount percentage based on the quantity purchased. Use orders table. Result set should show customerid, quantity and discount percentage.
---The discount should be applied as follows: 1 item: 3% Between 1 and 3 items : 5% Otherwise: 7% oxirgi savol.
...
INSERT INTO emmployees (EmployeeID, FirstName, LastName, DepartmentName, Salary, HireDate, Age, Email, Country) VALUES
...
--Write a query to find employees in the Employees table who have either Age < 25 or Salary > 60000.---11savol.

select *from emmployees
where Age<25 or Salary>6000


---Update the salary of an employee based on their department, increase by 10% if they work in 'HR' or EmployeeID = 5--12savol.
