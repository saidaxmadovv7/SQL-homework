...
-- 2. Test ma'lumotlarini kiritish
...
select a.Item,b.Item from Cart1 as a
...
select s.student_id,s.student_name,sub.subject_name,e.subject_name from Students as s
join Examinations as e on s.student_id=e.student_id
join Subjects as sub on e.subject_name=sub.subject_name
...
SELECT 
    ...
    s.student_name,
    sub.subject_name,
    COALESCE(COUNT(e.exam_id), 0) AS attended_exams
FROM Students AS s
