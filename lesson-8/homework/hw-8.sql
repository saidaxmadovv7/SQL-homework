...
select *,Case 
when  StockQuantity<30 then 'Low Stock' else 'Sufficent'end
from Products
...
left join Invoices  as I on O.CustomerID=I.CustomerID
where  O.OrderDate>='2023-1-1' and O.OrderDate<'2023-2-1'
...
select *from (
select District_id,District_name,Population,year  from city_population) as CP
pivot (max(year)for District_name  in ([2012],[2013])) as pvt
