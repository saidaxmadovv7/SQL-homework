...
INSERT INTO Ordders (CustomerID, OrderID, DeliveryState, Amount) VALUES
...
select FIRST_NAME as first_name,len(FIRST_NAME) as len_name    from Employees
...
SELECT COALESCE(id, ssn, passportid, itin) AS FirstNonNull
FROM person
...
SELECT STRING_AGG(SequenceNumber, ', ') AS concatenated_values
FROM DMLTable
...
SELECT p.JobDescription,
       most.SpacemanID AS MostExperiencedID,
       most.  SpacemanID AS MostYears,
       least.SpacemanID AS LeastExperiencedID,
       least.SpacemanID AS LeastYears
FROM Personal p
CROSS APPLY (
    SELECT TOP 1 SpacemanID
    FROM Personal
    WHERE JobDescription = p.JobDescription
    ORDER BY SpacemanID DESC
) AS most
CROSS APPLY (
    SELECT TOP 1 SpacemanID
    FROM Personal
    WHERE JobDescription = p.JobDescription
    ORDER BY SpacemanID ASC
) AS least
GROUP BY p.JobDescription, most.SpacemanID, most.SpacemanID, least.SpacemanID
...
DECLARE @eq NVARCHAR(50), @sql NVARCHAR(100), @result INT;

DECLARE cur CURSOR FOR
SELECT Equation FROM Equations

OPEN cur;
FETCH NEXT FROM cur INTO @eq;

WHILE @@FETCH_STATUS = 0
BEGIN
    SET @sql = N'SELECT ' + @eq;
    EXEC sp_executesql @sql, N'@res INT OUTPUT', @res=@result OUTPUT;
    PRINT @eq + ' = ' + CAST(@result AS NVARCHAR(10));
...
--------------------------------------------------------------------------------
SELECT CAST(BirthDay AS date) AS BirthDate,
       STRING_AGG(StudentName, ', ') AS Students
FROM Student
GROUP BY CAST(BirthDay AS date)
HAVING COUNT(*) > 1
ORDER BY BirthDate
